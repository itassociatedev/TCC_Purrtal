...
@php
        ...
        // 🟢 DYNAMIC BRANCH ADDRESS RESOLUTION
        $branchName = strtoupper(trim($po->purchaseRequest->branch ?? 'MAKATI'));

        $branchAddresses = [
            'MAKATI'     => 'Makati City, Metro Manila',
            'GREENHILLS' => 'Greenhills, San Juan City, Metro Manila',
            'ALABANG'    => 'Alabang, Muntinlupa City, Metro Manila'
        ];

        $displayAddress = $branchAddresses[$branchName] ?? ($po->purchaseRequest->branch ?? 'Makati City, Metro Manila');
@endphp
    ...
    <table class="master-header">
        <tr>
            <td style="width: 25%;">
                <div style="display: flex; align-items: center; gap: 8px;">
                    ...
                    <div>
                        ...
                        <div style="color: #6b7280; font-size: 10px; margin-top: 1px;">{{ $displayAddress }}</div>
                    </div>
                </div>
            </td>
            ...
            <td style="width: 20%; text-align: right; vertical-align: middle;">
                ...
                <div style="font-weight: bold; margin-top: 2px; font-size: 12px;">{{ $po->po_number }}</div>
                <div style="font-size: 10px; color: #4b5563; margin-top: 4px;">
                    Date: {{ \Carbon\Carbon::parse($po->po_date)->format('F d, Y') }}<br>
                    {{ $displayAddress }}
                </div>
            </td>
        </tr>
